 ...
 */
export default function CelestialSphere({ viewMode, selectedLocation, time }: CelestialSphereProps) {
    ...
    useEffect(() => {
        if (viewMode === 'orbit' && selectedLocation &&
        ...
        if (viewMode === 'orbit') {
            ...
            if (geom) {
                ...
                linePairs.forEach(([si, ei], idx) => {
                    ...
                    const off = idx * 6
                    arr[off] = sox; arr[off + 1] = soy; arr[off + 2] = soz
                    arr[off + 3] = eox; arr[off + 4] = eoy; arr[off + 5] = eoz
                })
    ...
    const linesMesh = useMemo(() => {
        ...
        linePairs.forEach(([si, ei], idx) => {
            ...
            const off = idx * 6
            verts[off] = s[0]; verts[off + 1] = s[1]; verts[off + 2] = s[2]
            verts[off + 3] = e[0]; verts[off + 4] = e[1]; verts[off + 5] = e[2]
        })
    ...
    useFrame((_state, delta) => {
        ...
        if (anim.isAnimating && selectedLocation) {
            ...
            const [tx, ty, tz] = latLonToXYZ(selectedLocation.lat, selectedLocation.lon, 1.0)
            ...
            for (let i = 0; i < STARS.length; i++) {
                ...
                const miniSize = STARS[i].mag < 1 ? 0.003 : STARS[i].mag < 2 ? 0.0022 : 0.0015
            ...
            if (geom) {
                ...
                linePairs.forEach(([si, ei], idx) => {
                    const [sox, soy, soz] = starOriginals[si]
                    const [eox, eoy, eoz] = starOriginals[ei]
                    const [smx, smy, smz] = raDecToXYZ(STARS[si].ra, STARS[si].dec, MINI_RADIUS)
                    const [emx, emy, emz] = raDecToXYZ(STARS[ei].ra, STARS[ei].dec, MINI_RADIUS)
                    const off = idx * 6
        ...
        <group ref={groupRef}>
            {starOriginals.map((pos, i) => (
                <mesh
                    key={i}
                    position={pos}
                    ref={(el) => { starMeshRefs.current[i] = el }}
                >
