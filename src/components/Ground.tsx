...
export default function Ground() {
  ...
  const compassMarkers = useMemo(() => {
    ...
    return [
      ...
      { label: "W", azimuth: 270, color: "#aaaaaa", x: -R, z: 0 },
      {
        ...
        z: -R * Math.cos(Math.PI / 4),
      },
      {
        label: "SE",
        azimuth: 135,
        color: "#666666",
        x: R * Math.sin((3 * Math.PI) / 4),
        z: -R * Math.cos((3 * Math.PI) / 4),
      },
      {
        label: "SW",
        azimuth: 225,
        color: "#666666",
        x: R * Math.sin((5 * Math.PI) / 4),
        z: -R * Math.cos((5 * Math.PI) / 4),
      },
      {
        label: "NW",
        azimuth: 315,
        color: "#666666",
        x: R * Math.sin((7 * Math.PI) / 4),
        z: -R * Math.cos((7 * Math.PI) / 4),
      },
    ];
  }, []);

  // ── N/S/E/W 방위 라벨 캔버스 캐싱 ──
  const compassCanvases = useMemo(() => {
    return compassMarkers.map((marker) => {
  ...
  }, [compassMarkers]); // Dependency on compassMarkers

  // ── 방위각 도수 라벨 캔버스 캐싱 ──
  const degreeCanvases = useMemo(() => {
    const dict: Record<number, HTMLCanvasElement> = {};
    // Only create canvases for degrees that will actually be rendered (not major compass points)
  ...
  return (
    <group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]}>
        <ringGeometry args={[HORIZON_RADIUS * 0.9, HORIZON_RADIUS, 64]} />
        <meshBasicMaterial
          color="#44aaee"
          transparent
          opacity={0.15}
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.01, 0]}>
        <ringGeometry
          args={[HORIZON_RADIUS * 0.97, HORIZON_RADIUS * 1.03, 128]}
        />
        <meshBasicMaterial
          color="#3399aa"
          transparent
      ...
      {compassMarkers.map((marker, i) => (
        <sprite
          key={marker.label}
          position={[marker.x, marker.label.length === 1 ? 12 : 6, marker.z]}
          scale={[
            marker.label.length === 1 ? 16 : 10,
            marker.label.length === 1 ? 8 : 5,
            1,
          ]}
        >
          <spriteMaterial
            ...
            depthTest={false}
          >
            <canvasTexture attach="map" image={compassCanvases[i]} />
          </spriteMaterial>
        </sprite>
      ))}
      ...
      {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((deg) => {
        ...
        return (
          <sprite key={`deg-${deg}`} position={[x, 3, z]} scale={[8, 4, 1]}>
            <spriteMaterial transparent opacity={0.4} depthTest={false}>
              <canvasTexture attach="map" image={degreeCanvases[deg]} />
            </spriteMaterial>
          </sprite>
        );
      })}
